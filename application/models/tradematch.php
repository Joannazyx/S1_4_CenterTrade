...
class TradeMatch extends CI_Controller {
	...
	public function __construct() {
		...
		foreach ($prices as $item) {
			$this->yesterday_end_price[$item['StockID']] = $item['StartPrice'];
		}
...
*/
		...
		$tmp4 = 'create view sellmap as
					select minp, max(CommissionTime) as CommissionTime, StockID from minsell
					group by StockID';
...
//		$this->pair['sell'] = $sellCmd->row_array();
	...
	protected function checkMatchness($Sid, $buyp, $sellp) {
		...
		if ($buyp < $sellp)
			return false;
		if ($buyp < $this->yesterday_end_price[$Sid] - $lat)
			return false;
		if ($sellp > $this->yesterday_end_price[$Sid] + $lat)
			return false;
		return true;
	}
	...
	protected function restrictRange($Sid, $p) {
		...
		if ($this->yesterday_end_price[$Sid] - $lat > $p)
			$p = $this->yesterday_end_price[$Sid] - $this->latitude;
		if ($this->yesterday_end_price[$Sid] + $lat < $p)
			$p = $this->yesterday_end_price[$Sid] + $this->latitude;
		return $p;
	}
		...
		for ($iterb=0, $iters=0, $iter=0,
			...
			*/
			if (! $this->checkMatchness($this->pair['buy'][$iterb]['StockID'], $p1, $p2)) {
				$this->pair['debug'][$iter]['debugInfo'] = '买方最高出价 低于 卖方最低出价 或者和涨跌停不符，不成交';
...
/*			/
			...
			*/
			...
			if (!$this->db->query($sqlb) || !$this->db->query($sqls)) {
				//  error handling
				throw new Exception('Update 待处理指令数据库出错!!!');
			}
		}
